...
CREATE_A=$(./target/debug/cli create --image-path "$TEST_IMAGE" --enable-all-namespaces --async-mode -- /bin/sleep 3600 2>&1)
...
CREATE_B=$(./target/debug/cli create --image-path "$TEST_IMAGE" --enable-all-namespaces --async-mode -- /bin/sleep 3600 2>&1)
...
LIFECYCLE_CONTAINER=$(./target/debug/cli create --image-path "$TEST_IMAGE" --enable-all-namespaces --async-mode -- /bin/sleep 60 2>&1 | grep "Container ID:" | tail -1 | awk '{print $NF}')
