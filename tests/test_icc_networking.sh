...
if [ ! -z "$CONTAINER_A" ] && [ ! -z "$CONTAINER_B" ]; then
    ...
    if [ ! -z "$IP_B" ]; then
        ...
        if ./target/debug/cli exec $CONTAINER_A -c "ping -c 1 -W 2 $IP_B" >/dev/null 2>&1; then
            echo -e "${GREEN}PASSED${NC}"
            ((PASSED++))
        else
            echo -e "${RED}FAILED${NC}"
    ...
    if [ ! -z "$NAME_B" ]; then
        if ./target/debug/cli exec $CONTAINER_A -c "nslookup $NAME_B 10.42.0.1" 2>&1 | grep -q "$IP_B"; then
            echo -e "${GREEN}PASSED${NC}"
            ((PASSED++))
        else
            echo -e "${RED}FAILED${NC}"
